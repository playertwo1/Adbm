...
**Novo planejamento:** [Stomach Vacuum e implementação das 10 telas AMOLED](#13-stomach-vacuum-e-implementação-das-10-telas-amoled). Escopo aprovado para o roadmap; implementação e validação pendentes. As imagens são referências visuais, não funcionalidades já entregues.
...
## 13. Stomach Vacuum e implementação das 10 telas AMOLED

### 13.1 Objetivo, referências e regras de implementação

Transformar o programa de Stomach Vacuum em uma experiência guiada pela técnica, conforto e execução real. Implementar as dez telas de referência como interfaces funcionais, com navegação, dados persistidos, controles e integrações reais. Preservar os requisitos e pendências das seções anteriores, especialmente a proteção do progresso.

Referências: [pasta das dez imagens](assets/design/coreflow-s25-ultra), versionadas no commit `114366b`. Todos os itens desta seção estão **planejados e pendentes**.

- Recriar os componentes em interface responsiva; não usar uma imagem de tela inteira como interface clicável.
- Unificar textos, ícones e navegação que variam entre os mockups. Proposta de destinos fixos: Hoje, Programas, Pausas, Evolução e Perfil. Vácuo livre e Modo Discreto acessíveis pelas ações rápidas; Mindfulness acessível por Programas e Hoje.
- Os números, fotos de perfil, gráficos e estados de conexão das imagens são ilustrativos. Usar dados reais ou estados vazios; nunca preencher histórico ou conexão fictícios.
- As imagens não definem protocolos de exercício. Revisar orientações antes de incorporá-las, inclusive as dicas adicionadas pela geração de imagens.
- A Pausa de Resposta já existente tem fluxo próprio de três etapas; preservar esse roteiro. Não transformá-la em faixa de áudio fictícia apenas porque o mockup apresenta controles de áudio.
- Frequência cardíaca, passos, sono e calorias aparecem no mockup de Perfil, mas não são comprovados pela integração atual de vibração do Watch. Não exibir como recursos ativos: dependem de escopo de integração próprio.

### 13.2 Stomach Vacuum — correções e evolução

Diagnóstico no `index.html`: programa `id: '3'`, oito fases, uma sessão diária; retenções de 10–12s a 30s, 3 a 5 séries e metas de 5 a 7 dias. A carga aumenta pela contagem de dias, sem avaliação da execução. As fases chamadas de semanas avançam por dias praticados acumulados, não por semanas de calendário.

| ID / prioridade | Trabalho planejado | Critério de aceite |
| :--- | :--- | :--- |
| V1 / P0 | Corrigir o atalho Vácuo rápido: referência a `AppState.vacuum` incompatível com `AppState.vacuo`; conferir também a função de início chamada | Atalho abre e inicia a sessão correta sem erro; toque repetido não cria sessões concorrentes |
| V2 / P0 | Substituir os 10 minutos fixos registrados no término do vácuo livre web por tempo efetivamente praticado; harmonizar contabilização web/nativa | Pausas não acrescentam prática; etapas puladas não acrescentam duração prevista; uma conclusão gera um único registro |
| V3 / P1 | Criar tutorial de técnica, postura, sequência respiratória e diferença entre vácuo e bracing; revisar linguagem de expiração e retenção | Tutorial pode ser revisto antes da sessão; instruções visuais, voz e etapas concordam; conteúdo técnico revisado antes da entrega |
| V4 / P1 | Adicionar “Encerrar retenção” e “Mais descanso”, além de pausar, retomar e encerrar sessão | Encerrar retenção conduz à recuperação; aumentar descanso atualiza timer e estimativa; nenhuma ação reinicia uma apneia inesperadamente |
| V5 / P1 | Avaliação pós-sessão: Confortável, Difícil ou Interrompi, com checagem de técnica e conforto | Resposta persistida por sessão; ausência de resposta não equivale a execução confortável; interrupção não vira conclusão integral |
| V6 / P1 | Substituir avanço automático de carga por sugestão explicável de manter, reduzir ou avançar, considerando execução e feedback | Usuário pode repetir a etapa; avanço não decorre apenas de dias ou recorde de retenção; critérios e limites técnicos documentados antes de codificar |
| V7 / P1 | Agenda com dias escolhidos de prática e recuperação; distinguir etapa, semana de calendário e dias cumpridos | Descanso planejado não aparece como falha; alteração de agenda preserva histórico; rótulos não prometem oito semanas exatas se houver repetição |
| V8 / P1 | Histórico de séries feitas, retenção efetiva, recuperação, interrupções e conforto relatado | Resumo confere com os eventos da sessão; separar tempo total de sessão, tempo de retenção e minutos de prática; sem inferir qualidade por sensores inexistentes |
| V9 / P1 | Player AMOLED próprio, integrado ao motor existente: fase, contador, série, próxima etapa e controles próximos ao polegar | Interface reflete o estado real do motor; recuperação após bloqueio, segundo plano e pausa é coerente no Android e no fallback web |
| V10 / P0 | Migrar dados com cópia anterior e compatibilidade de backup; manter posição e histórico existentes | Atualização e importação não apagam progresso nem inventam avaliações antigas; campos novos opcionais em backups anteriores |

Não aumentar tempos para tornar o programa mais avançado sem revisão da dosagem. A literatura sobre hipopressivos tem limitações e não valida automaticamente o protocolo específico do app. Referência de revisão: [Effects of Hypopressive Techniques on the CORE Complex](https://pubmed.ncbi.nlm.nih.gov/40565470/). Distinguir hipótese de produto, instrução técnica revisada e evidência clínica.

### 13.3 Dez telas — comportamento funcional obrigatório

| Tela / referência | Botões e opções a implementar | Aceite |
| :--- | :--- | :--- |
| 01 — [Onboarding](assets/design/coreflow-s25-ultra/01-onboarding.png) | Selecionar objetivo, Continuar, Voltar e concluir personalização; definir etapas seguintes antes da implementação | Objetivo persiste e orienta sugestões; conclusão leva a Hoje; instalação atualizada preserva usuários e progresso |
| 02 — [Hoje](assets/design/coreflow-s25-ultra/02-home.png) | Editar meta, abrir recomendação, Começar agora, atalhos Vácuo/Pausa/Kegel/Meditar, lembretes e acesso ao perfil | Métricas vêm do diário; atalhos chegam ao módulo certo; recomendação usa programa e posição reais; definir regra transparente para quem ainda não tem programa |
| 03 — [Vácuo](assets/design/coreflow-s25-ultra/03-vacuo-player.png) | Selecionar postura e parâmetros antes de iniciar; iniciar/pausar/retomar, encerrar retenção, pular, recuperar e encerrar; voz, hápticos e Watch | Parâmetros ficam coerentes durante sessão; mudanças de postura não alteram exercício ativo silenciosamente; aplica V1–V10 |
| 04 — [Modo Discreto](assets/design/coreflow-s25-ultra/04-modo-discreto.png) | Sem áudio, posição, intensidade, prévia da sequência e Iniciar sessão | Cada escolha corresponde a uma configuração real e revisada; modo silencioso respeitado; sessão pode ser pausada e encerrada e registra o executado |
| 05 — [Pausas Ativas](assets/design/coreflow-s25-ultra/05-pausas-ativas.png) | Filtros por região, abrir exercício, Ver todos, Começar pausa e acesso às práticas relacionadas | Filtros alteram resultados; cards abrem instruções e sessão correspondente; recomendação tem regra definida e estado sem dados |
| 06 — [Programas](assets/design/coreflow-s25-ultra/06-programas.png) | Abrir Bracing/Kegel/Vácuo/Mindfulness, continuar treino do dia, consultar progresso | Cada card aponta ao programa correto; duração e semanas derivadas do programa real, incluindo Bracing de oito semanas onde já implementado; nenhum percentual ilustrativo |
| 07 — [Detalhe do programa](assets/design/coreflow-s25-ultra/07-programa-detalhe.png) | Ver etapas, exercícios e histórico; Iniciar treino guiado; repetir etapa e ajuste manual com prévia | Player abre a semana/sessão exibida; estimativa soma etapas reais; ajuste preserva diário e não cria treinos passados |
| 08 — [Mindfulness](assets/design/coreflow-s25-ultra/08-mindfulness-player.png) | Selecionar faixa disponível, play/pausa, ±15s, buscar posição, próxima faixa e preferências sonoras/hápticas | Controles atuam no áudio real, inclusive bloqueado; faixa ausente tem erro recuperável; modo silencioso explicita se silencia avisos ou também a narração; Pausa de Resposta mantém seu fluxo próprio |
| 09 — [Evolução](assets/design/coreflow-s25-ultra/09-evolucao.png) | Trocar período, explorar dias, abrir conquistas e histórico, exportar relatório | Totais e gráficos concordam; tratar período vazio e comparação sem base; insights só aparecem quando sustentados pelos dados, sem alegar rendimento por horário sem medida |
| 10 — [Perfil e preferências](assets/design/coreflow-s25-ultra/10-perfil-preferencias.png) | Nome/meta, lembretes, voz, vibração, tema, acessibilidade, status/teste Watch, backup/exportação/importação e gerenciamento de dados | Preferências persistem; permissões têm retorno real; relógio indisponível não aparece conectado; importação tem prévia/cancelamento; exclusão exige confirmação e escopo claro |

Para qualquer ícone ou ação adicional aproveitada das imagens, documentar destino, alteração de estado e retorno ao usuário. Não entregar botão decorativo com aparência de funcional. Estados indisponíveis precisam de motivo visível; cancelamento retorna ao contexto anterior.

### 13.4 Sistema visual e integração no aplicativo

- [ ] Criar tokens compartilhados: fundo `#000000`, superfícies `#0A0A0B` e `#111214`, texto claro, verde/menta principal, ciano secundário e âmbar para estados apropriados; reduzir brilhos excessivos dos mockups.
- [ ] Padronizar tipografia, ícones, espaçamentos, cards, seletores, modais, botões e barra inferior; garantir contraste, foco, rótulos acessíveis e alvos de toque de pelo menos 48dp.
- [ ] Adaptar ao Galaxy S25 Ultra com área útil ampla, recorte da câmera e insets reais das barras do sistema. Usar unidades lógicas e layout responsivo, sem fixar a aplicação em 1440×3120 pixels.
- [ ] Respeitar tamanho de fonte do sistema, TalkBack e redução de movimento; animações respiratórias não podem ser a única indicação da fase.
- [ ] Validar também telas menores, teclado aberto e rotação; manter ação principal acessível sem sobreposição à navegação por gestos.
- [ ] Unificar estado entre telas, WebView, serviço de treino/áudio e Galaxy Watch; impedir reprodução ou sessões concorrentes incompatíveis.
- [ ] Atualizar de forma consistente `index.html` e `app/src/main/assets/index.html`; garantir assets locais para os fluxos essenciais offline.
- [ ] Preservar IDs de programas, histórico, conquistas, lembretes e contratos da bridge Android durante a reorganização visual.

### 13.5 Sequência de entrega e validação

1. **Fundação:** corrigir V1/V2, definir dados de sessão, migração e regras de progressão; revisar conteúdo do vácuo. Registrar comportamento atual antes de trocar a interface.
2. **Primeira entrega funcional:** tokens visuais e telas 03/06/07 para Vácuo, Programas e detalhe; tutorial, recuperação e feedback de sessão integrados.
3. **Entrada e preferências:** telas 01/02/10, navegação unificada, metas, lembretes, backup e Watch com estados reais.
4. **Demais práticas:** telas 04/05/08 ligadas aos exercícios, áudios e Pausa de Resposta existentes.
5. **Evolução:** tela 09 com dados reais e novas métricas do vácuo; fechar consistência visual e funcional das dez telas.
6. **Entrega Android:** compilar, validar atualização preservando dados, testar em S25 Ultra e Watch quando disponíveis e documentar limitações efetivamente encontradas antes da publicação.

Critérios de conclusão:

- [ ] Cada botão, card, opção e gesto tem fluxo executável e verificado, incluindo Voltar do Android, cancelamento, vazio, erro e permissão negada.
- [ ] Testes de regressão cobrem contabilização, duplicidade de callbacks, etapas puladas, interrupção, pausa/retomada, troca de dia e migração/importação de dados anteriores.
- [ ] Sessões web e nativas preservam a mesma semântica; tela bloqueada e segundo plano não reiniciam retenções nem duplicam registros.
- [ ] Comparação visual das dez telas com suas referências, com desvios intencionais documentados para consistência, acessibilidade e funcionalidades existentes.
- [ ] Não declarar acessibilidade, desempenho, consumo de bateria ou integração física validados apenas por imagens; registrar teste real e pendências.
- [ ] Nenhum dado demonstrativo, controle sem efeito ou indicador de recurso inexistente na versão entregue.
