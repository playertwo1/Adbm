...
- Regressão `diagnostics/e13-program-retention-exit.test.cjs` escrita e observada RED (controle ausente), depois GREEN nas duas cópias HTML. A UI da sessão programada agora oferece `Encerrar retenção` apenas durante `vacuo` com bridge nativa disponível, tanto rodando quanto pausada, acionando a saída segura em vez do skip genérico; desaparece em `retorno`. `scripts/check.sh` PASS (inclui builds app/Wear), HTMLs idênticos, `git diff --check` limpo.
- APK debug instalado com `adb install -r` no Pixel_9, preservando dados. Na sessão Stomach Vacuum semana 1, o botão apareceu ao vivo na retenção; toque com 9s restantes → estado persistido `paused`, passo 4 `retorno` com 5s, `retentionElapsedSeconds=1`, `retentionInterruptedSeries=[1]`; captura `C:\Users\notefael\AppData\Local\hermes\cache\scratch\adbm-e13-safe-exit-result.png` mostra **RETORNO CONTROLADO**, orientação de soltar a sucção antes de inspirar e controle Continuar. Cancelamento pela UI retornou o serviço a `idle`; logcat do cenário sem `FATAL EXCEPTION`, `ERROR:CONSOLE` ou `Uncaught`.
- Evidência cobre a transição programada e o registro nativo parcial, **não** a persistência no diário após concluir a sessão, a retomada sem apneia, Watch/telefone físico/TalkBack ou os demais itens E04. Não marcar E13 concluída nem publicar esta branch parcial.

## 2026-09-25 — E13 / E04 fases nativas observadas e E12 publicada em main

- `origin/main` recebeu a E12 no merge `716c297bd051686650914d38684d54967ee90743` (pais `284a0b9`/`77bf1e0`), `scripts/check.sh` PASS na árvore integrada, SHA remoto confirmado. Auditoria independente E12 continua adiada; commit `[skip actions]` sem execução de Actions para esse SHA na consulta.
- Na worktree E13 `work/e13-after-e12-20260925`, rodei sessão programada Stomach Vacuum de novo no APK combinado. Polling do estado persistido do serviço registrou transições `idle` → `prepara` (passo 0) → `inspira` (1) → `expira` (2) → `vacuo` (3) → `retorno` (4). Screenshot `C:\Users\notefael\AppData\Local\hermes\cache\scratch\adbm-e13-return-live.png` mostra **RETORNO CONTROLADO**, timer 00:05, instrução de soltar a sucção antes de inspirar e próximo descanso. Descanso (passo 5) havia sido observado no smoke anterior. Interrompi via X/confirmação, serviço terminou em `idle`.
- Esta evidência fecha a lacuna de **observação das seis fases no AVD**, mas ainda não comprova todos os caminhos de pausa/retomada, aparelho físico, Watch ou acessibilidade e não conclui E04/E13 por inferência. Próximo item: `Encerrar retenção` na sessão programada e persistência associada.

## 2026-09-25 — E13 sobre E12 / APK no Pixel_9 — SMOKE PARCIAL

- Branch/worktree atual: `work/e13-after-e12-20260925` baseada diretamente no commit E12 publicado `77bf1e0aa9218933bfac0b1365fafb78f4abdc59` (não em `origin/main`). A primeira tentativa E13 baseada em `origin/main` executou somente smoke do player rápido; não incluía todo o código E12, portanto foi substituída no AVD pelo build desta branch combinada. Nenhum merge/push E13 ou `main`.
- Regressão E13/E04 `diagnostics/e13-e04-vacuum-phases.test.cjs`: RED com `phase` indefinida em posicionamento/retorno; `prepara` e `retorno` adicionadas nos dois HTMLs; GREEN. `scripts/check.sh` → `CHECK PASS` com app/Wear debug, HTMLs idênticos, `git diff --check` limpo. APK debug empacota exatamente o `app/src/main/assets/index.html` da worktree.
